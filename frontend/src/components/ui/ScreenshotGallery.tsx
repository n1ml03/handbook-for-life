  ...
  ({
    ...
    const imageUrl = useMemo(() => {
      if (useOptimizedUrls && documentId) {
        ...
        if (!screenshot?.data || screenshot.data.trim().length === 0)
          return null;
...
export const ScreenshotGallery = React.memo(
  ({
    ...
    const screenshotUrls = useMemo(() => {
      if (useOptimizedUrls && documentId) {
        ...
        return screenshots.map(
          (_, index) =>
            `/api/images/document/${documentId}/screenshot/${index}`,
        );
      }
    ...
    const previousImage = useCallback(() => {
      if (lightboxIndex !== null) {
        setLightboxIndex(
          lightboxIndex === 0 ? screenshotUrls.length - 1 : lightboxIndex - 1,
        );
      }
    }, [lightboxIndex, screenshotUrls.length]);
