...
const OptimizedCardImage = React.memo<{
  ...
  const containerClasses = cn(
    ...
    config.size
      ...
      : aspectRatioClasses[config.aspectRatio || "video"],
    config.className,
    className,
  );
...
export const MultiLanguageCard = React.memo<MultiLanguageCardProps>(
  ({
    ...
    const densityConfig = useMemo(() => {
      const configs = {
        compact: {
          ...
          textSpacing: "space-y-1 sm:space-y-2",
        },
        comfortable: {
          padding: "p-4 sm:p-5 md:p-6",
          spacing: "space-y-3 sm:space-y-4",
          textSpacing: "space-y-2 sm:space-y-3",
        },
        spacious: {
          padding: "p-6 sm:p-7 md:p-8",
          spacing: "space-y-4 sm:space-y-5",
          textSpacing: "space-y-3 sm:space-y-4",
        },
      };
        ...
        if (hasImage && (isImageLeft || isImageRight)) {
          return (
            ...
            >
              ...
              >
                <div
                  className={cn(sideContent ? "grid grid-cols-1 gap-4" : "")}
                >
                  <LanguageDisplay
                    names={names}
                    primaryLanguage={primaryLanguage}
    ...
    return <Card className={cardClasses.base}>{cardContent}</Card>;
  },
);
