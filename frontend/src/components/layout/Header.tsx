...
const useDropdownState = () => {
  ...
  const handleMouseLeave = useCallback((itemId: string) => {
    hoverTimeouts.current[itemId] = setTimeout(() => {
      setOpenDropdowns((prev) =>
        prev[itemId] ? { ...prev, [itemId]: false } : prev,
      );
      delete hoverTimeouts.current[itemId];
    }, HOVER_DELAY);
  }, []);
          ...
          {children.map((child) => (
            ...
            >
              ...
              {child.badge && (
                <Badge
                  variant="secondary"
                  className="ml-auto text-xs px-2 py-1 bg-accent-pink/20 text-accent-pink border-accent-pink/30 font-semibold"
                >
                  {child.badge}
                </Badge>
              )}
...
export const Header = memo(function Header({ className }: HeaderProps) {
  ...
  const { openDropdowns, handleMouseEnter, handleMouseLeave } =
    useDropdownState();
  ...
  const renderMenuItem = useCallback(
    (item: MenuItem, isMobile: boolean, index: number) => {
  ...
  return (
    ...
    >
      <div
        className="modern-container safe-area-inset"
        style={{ overflow: "visible" }}
      >
        <div
          className="flex items-center justify-between min-h-[56px] md:min-h-[64px] py-2"
          style={{ overflow: "visible" }}
        >
          {/* Logo */}
          <Link
            to="/home"
            className="header-logo touch-target"
            aria-label="Handbook"
          >
          ...
          >
            {MENU_ITEMS.map((item, index) =>
              renderMenuItem(item, false, index),
            )}
          </nav>
        ...
        {mobileMenuOpen && (
          ...
          >
            <nav className="mobile-menu-nav space-y-1 custom-scrollbar">
              {MENU_ITEMS.map((item, index) =>
                renderMenuItem(item, true, index),
              )}
            </nav>
          </div>
        )}
