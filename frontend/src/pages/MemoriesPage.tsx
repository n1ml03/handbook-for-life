...
export default function MemoriesPage() {
  ...
  const fetchEpisodes = useCallback(async () => {
    try {
      ...
      const params = {
        ...
        ...(filterValues.related_entity_type && {
          entityType: filterValues.related_entity_type,
        }),
        ...(filterValues.related_entity_id && {
          entityId: Number(filterValues.related_entity_id),
        }),
        ...(debouncedSearch && { search: debouncedSearch }),
      };

      const response = await episodesApi.getEpisodes(params);
      const episodeData = safeExtractArrayData<Episode>(
        response,
        "episodes API",
      );

      // Convert episodes to memories
      const memoryData = episodeData.map((episode: Episode) =>
        episodeToMemory(episode),
      );
  ...
  const memoizedFilterOptions = useMemo(() => {
    const characters = [
      ...new Set(memories.map((m) => m.characters).flat()),
    ].sort();
  ...
  const filterFields = useMemo(
    () =>
      createMemoriesFilterConfig(
        episodeTypes,
        memoizedFilterOptions.characters,
        memoizedFilterOptions.versions,
      ),
    [memoizedFilterOptions.characters, memoizedFilterOptions.versions],
  );

  return (
    <PageLoadingState isLoading={loading} message="Loading episodes...">
      <div className="modern-page">
        <div className="modern-container-lg">
          ...
          >
            <h1 className="modern-page-title">Episodes Gallery</h1>
          </motion.div>
          ...
          {!loading && filteredMemories.length === 0 && (
            ...
            >
              ...
              <p className="text-muted-foreground mb-6">
                {filterValues.search
                  ? "Try adjusting your search terms or clear the search to see all episodes."
                  : "Try adjusting your filters or clear them to see all episodes."}
              </p>
              <motion.button
