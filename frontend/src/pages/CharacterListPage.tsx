...
import { ChevronLeft, ChevronRight, User, Calendar, Mic } from "lucide-react";
...
export default function CharacterListPage() {
  ...
  const characters = useMemo(() => {
    return safeExtractArrayData<Character>(
      charactersResponse,
      "characters API",
    );
  }, [charactersResponse]);
  ...
  const filteredAndSortedCharacters = useMemo(() => {
    const filtered = multiLanguageCharacters.filter((character) => {
      ...
      if (
        filterValuesWithDebouncedSearch.is_active &&
        originalChar.is_active.toString() !==
          filterValuesWithDebouncedSearch.is_active
      )
        return false;
      if (
        filterValuesWithDebouncedSearch.search &&
        !searchInAllLanguages(character, filterValuesWithDebouncedSearch.search)
      )
        return false;
      if (
        filterValuesWithDebouncedSearch.birthday_month &&
        originalChar.birthday
      ) {
    ...
    return filtered.sort((a, b) => {
      ...
      switch (sortState.sortBy) {
        ...
        case "id":
          ...
          break;
        case "birthday":
          aValue = originalA.birthday
            ? new Date(originalA.birthday).getTime()
            : 0;
          bValue = originalB.birthday
            ? new Date(originalB.birthday).getTime()
            : 0;
          break;
        case "height":
  ...
  const paginationData = useMemo(() => {
    const totalPages = Math.ceil(
      filteredAndSortedCharacters.length / itemsPerPage,
    );
    ...
    <PageLoadingState isLoading={isLoading} message="Loading character list...">
      <div className="modern-page">
        <div className="modern-container-xl">
          ...
          >
            <h1 className="modern-page-title">Character Collection</h1>
          </motion.div>
                    ...
                    <CharacterCard
                      ...
                      onClick={() =>
                        handleCharacterClick(
                          (character as typeof character & Character).id,
                        )
                      }
                    />
                  </motion.div>
