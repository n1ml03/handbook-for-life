...
const SwimsuitCard = React.memo(function SwimsuitCard({
  swimsuit,
}: SwimsuitCardProps) {
  const [currentView, setCurrentView] = useState<"before" | "after">("before");
  const characterName = swimsuit.character
    ? getLocalizedName(swimsuit.character, "en")
    : "Unknown";
  ...
  const header = (
    <div className="relative">
      ...
      >
        ...
        {(currentView === "before" && swimsuitImages.beforeImage) ||
        (currentView === "after" && swimsuitImages.afterImage) ? (
          <img
            src={
              currentView === "before"
                ? swimsuitImages.beforeImage
                : swimsuitImages.afterImage
            }
...
export default function SwimsuitPage() {
  ...
  useEffect(() => {
    const fetchSwimsuits = async () => {
      try {
        setLoading(true);
        const response = await swimsuitsApi.getSwimsuits({
          limit: 100,
          page: 1,
        });
        const responseData = safeExtractArrayData<Swimsuit>(
          response,
          "swimsuits API",
        );
  ...
  const filteredAndSortedSwimsuits = useMemo(() => {
    const filtered = multiLanguageSwimsuits.filter((swimsuit: any) => {
      if (filterValues.rarity && swimsuit.rarity !== filterValues.rarity)
        return false;
      const characterName = swimsuit.character?.name_en || "Unknown";
      if (filterValues.character && characterName !== filterValues.character)
        return false;
      // Use debounced search for better performance
      if (debouncedSearch && !searchInAllLanguages(swimsuit, debouncedSearch))
        return false;
      const stats = (swimsuit as any).stats || {
        pow: 0,
        tec: 0,
        stm: 0,
        apl: 0,
      };
      if (filterValues.minPow && stats.pow < Number(filterValues.minPow))
        return false;
      if (filterValues.minTec && stats.tec < Number(filterValues.minTec))
        return false;
      if (filterValues.minStm && stats.stm < Number(filterValues.minStm))
        return false;
      if (filterValues.minApl && stats.apl < Number(filterValues.minApl))
        return false;
      if (
        filterValues.hasSkills &&
        (!swimsuit.skills || swimsuit.skills.length === 0)
      )
        return false;
      ...
      if (
        ...
        !releaseDate.includes(filterValues.releaseYear as string)
      )
        return false;
      if (
        filterValues.version &&
        !String(swimsuit.id).includes(filterValues.version as string)
      )
        return false; // Simple version check
      return true;
    });
    ...
    multiLanguageSwimsuits,
    debouncedSearch,
    filterValues.rarity,
    filterValues.character,
    filterValues.minPow,
    filterValues.minTec,
    filterValues.minStm,
    filterValues.minApl,
  ...
  const totalPages = Math.ceil(
    filteredAndSortedSwimsuits.length / itemsPerPage,
  );
  ...
  const characters = useMemo(
    () =>
      [
        ...new Set(swimsuits.map((s) => s.character?.name_en || "Unknown")),
      ].sort(),
    [swimsuits],
  );
  ...
  const releaseYears = useMemo(
    () =>
      [
        ...new Set(
          swimsuits.map(
            (s) => (s.release_date_gl || "2023-01-01").toString().split("-")[0],
          ),
        ),
      ]
        .sort()
  ...
  const filterFields = useMemo(
    () =>
      createSwimsuitFilterConfig(rarities, characters, releaseYears, versions),
    [characters, rarities, releaseYears, versions],
  );
    ...
    <PageLoadingState isLoading={loading} message="Loading swimsuits...">
      <div className="modern-page">
        <div className="modern-container-lg">
          ...
          >
            <h1 className="modern-page-title">Swimsuit Gallery</h1>
          </motion.div>
