...
const convertToMultiLanguageItem = (
  item: any,
  type: ItemType,
): MultiLanguageItem & UnifiedItem => {
...
const ItemCard = React.memo(function ItemCard({
  ...
  item: UnifiedItem & MultiLanguageItem;
}) {
    ...
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <Badge
            variant="outline"
            className={cn("text-xs mb-2 shadow-xs", typeColor)}
          >
            {typeIcon}
...
export default function ItemsPage() {
  ...
  const rawUnifiedItems = useMemo(() => {
    ...
    if (Array.isArray(swimsuits) && swimsuits.length > 0) {
      items.push(
        ...swimsuits.map((swimsuit: Swimsuit) =>
    ...
    if (Array.isArray(accessories) && accessories.length > 0) {
      items.push(
        ...accessories.map((accessory: Item) =>
  ...
  const filterOptions = useMemo(() => {
    const uniqueRarities = [
      ...new Set(unifiedItems.map((item) => item.rarity).filter(Boolean)),
    ];
    const uniqueCharacters = [
      ...new Set(unifiedItems.map((item) => item.character).filter(Boolean)),
    ];
    return { uniqueRarities, uniqueCharacters };
  }, [unifiedItems]);

  // Create additional filters function for multi-language search
  const additionalFilters = useCallback(
    (item: any) => {
      // Type filter
      const typeMatch =
      ...
      // Rarity filter
      const rarityMatch =
        !filterValues.rarity || item.rarity === filterValues.rarity;

      // Character filter
      const characterMatch =
        !filterValues.character || item.character === filterValues.character;
  ...
  const filterFields: FilterField[] = useMemo(
    () => [
      ...
      },
      {
        ...
        options: filterOptions.uniqueRarities.map((r) => ({
          value: r!,
          label: r!,
        })),
      ...
      },
      {
        ...
        options: filterOptions.uniqueCharacters.map((c) => ({
          value: c!,
          label: c!,
        })),
  ...
  const handleSortChange = useCallback(
    (newSortBy: string, direction: SortDirection) => {
  ...
  return (
    <div className="modern-page">
      <div className="modern-container-lg">
        ...
        >
          <h1 className="modern-page-title">Items Collection</h1>
        </motion.div>
