...
const AccessoryCard = React.memo(function AccessoryCard({
  accessory,
}: AccessoryCardProps) {
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      case "SSR":
        return "from-yellow-400 to-orange-500 text-black";
      case "SR":
        return "from-purple-400 to-pink-500 text-white";
      case "R":
        return "from-blue-400 to-cyan-500 text-white";
      default:
      ...
      <div className="relative z-10">
        ...
        {accessory.stats && (
          <div className="grid grid-cols-2 gap-responsive mb-4">
            {Object.entries(accessory.stats).map(
              ([stat, value]) =>
                value > 0 && (
                  <motion.div
                    key={stat}
                  ...
                  >
                    <span
                      className={`uppercase font-bold text-xs ${getStatColor(stat)}`}
                    >
                      {stat}
                    </span>
          ...
          <div className="mb-4 p-3 bg-gradient-to-r from-accent-cyan/10 to-accent-purple/10 rounded-xl border border-accent-cyan/20">
            <div className="flex justify-between items-center">
              ...
              <span className="text-lg font-bold bg-gradient-to-r from-accent-cyan to-accent-purple bg-clip-text text-transparent">
                {(accessory.stats.pow || 0) +
                  (accessory.stats.tec || 0) +
                  (accessory.stats.stm || 0) +
                  (accessory.stats.apl || 0)}
              </span>
            </div>
          </div>
        )}
...
export default function AccessoryPage() {
    ...
    const fetchAccessories = async () => {
      try {
        ...
        if (!Array.isArray(responseData)) {
          console.warn(
            "Expected array from items API, received:",
            responseData,
          );
          setAccessories([]);
          return;
        }
        const transformedData: ExtendedAccessory[] = responseData.map(
          (item: Item) => ({
            id: item.id.toString(),
  ...
  const filteredAndSortedAccessories = useMemo(() => {
    const filtered = multiLanguageAccessories.filter((accessory) => {
      if (filterValues.rarity && accessory.rarity !== filterValues.rarity)
        return false;
      if (filterValues.type && accessory.type !== filterValues.type)
        return false;
      if (
        filterValues.version &&
        typeof filterValues.version === "string" &&
        !accessory.id.toString().includes(filterValues.version)
      )
        return false;
      if (
        filterValues.search &&
        typeof filterValues.search === "string" &&
        !searchInAllLanguages(accessory, filterValues.search)
      )
        return false;
      if (
        filterValues.minPow &&
        (accessory.stats?.pow || 0) < Number(filterValues.minPow)
      )
        return false;
      if (
        filterValues.minTec &&
        (accessory.stats?.tec || 0) < Number(filterValues.minTec)
      )
        return false;
      if (
        filterValues.minStm &&
        (accessory.stats?.stm || 0) < Number(filterValues.minStm)
      )
        return false;
      if (
        filterValues.minApl &&
        (accessory.stats?.apl || 0) < Number(filterValues.minApl)
      )
        return false;
      return true;
    });
  ...
  const totalPages = useMemo(
    () => Math.ceil(filteredAndSortedAccessories.length / itemsPerPage),
    [filteredAndSortedAccessories.length, itemsPerPage],
  );
  const paginatedAccessories = useMemo(
    () =>
      filteredAndSortedAccessories.slice(
  ...
  return (
    <PageLoadingState isLoading={loading} message="Loading accessory list...">
      <div className="modern-page">
        <div className="modern-container-lg">
          ...
          >
            <h1 className="modern-page-title">Accessory Gallery</h1>
          </motion.div>
