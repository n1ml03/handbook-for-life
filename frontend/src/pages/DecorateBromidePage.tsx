...
import {
  createDecorBromideFilterConfig,
  bromideSortOptions,
...
export default function DecorateBromidePage() {
    ...
    const fetchBromides = async () => {
      try {
        ...
        if (!Array.isArray(bromideData)) {
          console.warn(
            "Expected array from bromides API, received:",
            bromideData,
          );
          setBromides([]);
          return;
  ...
  const allTypes = useMemo(
    () => [...new Set([...bromideTypes, ...decorationTypes])],
    [],
  );
  const allRarities = useMemo(
    () => [...new Set(bromides.map((b) => b.rarity))].sort(),
    [bromides],
  );
  const allCharacters = useMemo(
    () =>
      [
        ...new Set(
          bromides.map((b) => b.character).filter(Boolean) as string[],
        ),
      ].sort(),
    [bromides],
  );
  const allSources = useMemo(
    () => [...new Set(bromides.map((b) => b.source ?? b.category))].sort(),
    [bromides],
  );
  ...
  const filteredAndSortedBromides = useMemo(() => {
    const filtered = bromides.filter((bromide) => {
      ...
      // Character filter
      const characterValue = String(
        filterValuesWithDebouncedSearch.character || "",
      );
      ...
      if (sourceValue && (bromide.source ?? bromide.category) !== sourceValue)
        return false;

      // Effects filter
      if (
        filterValuesWithDebouncedSearch.hasEffects &&
        (!bromide.effects || bromide.effects.length === 0)
      )
        return false;

      // Character presence filter
      if (filterValuesWithDebouncedSearch.hasCharacter && !bromide.character)
        return false;

      // Version filter (check if ID contains version)
      const versionValue = String(
        filterValuesWithDebouncedSearch.version || "",
      );
  ...
  const totalPages = useMemo(
    () => Math.ceil(filteredAndSortedBromides.length / itemsPerPage),
    [filteredAndSortedBromides.length, itemsPerPage],
  );
  const paginatedBromides = useMemo(
    () =>
      filteredAndSortedBromides.slice(
    ...
    >
      <div className="modern-page">
        <div className="modern-container-lg">
          ...
          >
            <h1 className="modern-page-title">Deco-bromides Collection</h1>
          </motion.div>
          ...
          {filteredAndSortedBromides.length === 0 && !loading && (
            ...
            >
              ...
              <h3 className="text-2xl font-bold text-gray-300 mb-3">
                No bromides or decorations found
              </h3>
