...
const AdminPage = () => {
  ...
  const [documentCategories] = useState<DocumentCategory[]>(
    documentCategoriesData,
  );
  const {
    updateLogs,
    ...
    async (document: Document) => {
      ...
      try {
        ...
        await measureOperation(
          ...
          async () => {
            ...
            const isExistingDocument =
              editingDocument?.id && editingDocument.id > 0;
        ...
        if (error?.message?.includes("required fields")) {
          ...
          errorTitle = "Validation Error";
          errorMessage =
            "Invalid document data. Please check all fields and try again.";
  ...
  const handleSaveUpdateLog = useCallback(
    async (log: UpdateLog) => {
      // Show loading notification
      addNotification({
        type: "info",
      ...
      try {
        ...
        if (error?.message?.includes("required fields")) {
          ...
          errorTitle = "Validation Error";
          errorMessage =
            "Invalid update log data. Please check all fields and try again.";
        } else if (error?.status === 401) {
          errorTitle = "Authorization Error";
          errorMessage = "You are not authorized to perform this action.";
        } else if (error?.status === 409) {
  ...
  const handleDeleteUpdateLog = useCallback(
    async (logId: string) => {
  ...
  const getFilteredDocuments = useCallback(() => {
    ...
    return documents.filter(
      (doc) => doc.document_type === activeDocumentSection,
    );
                        ...
                        onKeyDown={(e) => {
                          ...
                          if (e.key === "ArrowLeft" && index > 0) {
                            ...
                            const prevButton = e.currentTarget.parentElement
                              ?.children[index - 1] as HTMLButtonElement;
                            prevButton?.focus();
                          } else if (
                            e.key === "ArrowRight" &&
                            ...
                            const lastButton = e.currentTarget.parentElement
                              ?.children[
