...
export function usePerformanceMonitor() {
  ...
  const endOperation = useCallback(
    (
      ...
      if (!startTime) {
        console.warn(
          `⚠️ No start time found for operation: ${operationName} (ID: ${operationId})`,
        );
        return;
      }
      ...
      if (import.meta.env.DEV) {
        ...
        console.log(
          `${emoji} Completed operation: ${operationName} in ${durationFormatted}ms`,
        );

        // Warn about slow operations (>2 seconds)
        if (duration > 2000) {
          console.warn(
            `🐌 Slow operation detected: ${operationName} took ${durationFormatted}ms`,
          );
        }
      }
  ...
  useEffect(() => {
      ...
      entries.forEach((entry) => {
        ...
        if (entry.entryType === "layout-shift") {
          ...
          if (layoutShift.value > 0.1) {
            console.warn(
              `⚠️ Layout shift detected: ${layoutShift.value.toFixed(4)} - ${layoutShift.sources?.[0]?.node || "Unknown element"}`,
            );
          }
        }
    ...
    try {
      observer.observe({
        entryTypes: ["measure", "navigation", "layout-shift", "longtask"],
      });
    } catch {
    ...
    const handleScrollEnd = () => {
      ...
      if (fps < 30) {
        console.warn(
          `🎯 Scroll performance warning: ${fps.toFixed(1)} FPS during ${scrollDuration.toFixed(2)}ms scroll`,
        );
      } else {
        console.log(
          `✅ Scroll performance: ${fps.toFixed(1)} FPS during ${scrollDuration.toFixed(2)}ms scroll`,
        );
      }
    };
  ...
  const measureScrollPerformance = useCallback((elementId?: string) => {
    ...
    const handleScrollEnd = () => {
      if (isScrolling) {
        ...
        console.log(
          `📊 Scroll Performance - Duration: ${scrollDuration.toFixed(2)}ms, FPS: ${fps.toFixed(1)}`,
        );
      }
    };
